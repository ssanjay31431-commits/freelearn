        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-slate-400 space-y-0.5">
                        <div>Priority: <span className="text-slate-200 uppercase font-semibold">{item.priority}</span></div>
                        {item.requirements && <div>Reqs: <span className="text-slate-300 italic">"{item.requirements}"</span></div>}
                        {item.referenceFile && <div>File: <span className="text-cyan-400">{item.referenceFile}</span></div>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-6">
                      ...
                      <div className="text-right">
                        <div className="text-lg font-extrabold text-white">₹{item.price * item.quantity}</div>
                    ...
                    {savedForLater.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/60 border border-slate-800">
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-indigo-600/30 border border-indigo-500 text-indigo-200 text-xs font-semibold hover:bg-indigo-600 hover:text-white transition-all"
                        >
                          Move to Cart
                        </button>
                      </div>
            ...
            <div className="lg:col-span-4">
              <div className="glass-panel rounded-3xl p-6 border border-slate-700/80 sticky top-28 space-y-6">
                  ...
                  <form onSubmit={handleApply} className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-9 pr-3 py-2.5 text-xs text-white uppercase placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                      />
                    </div>
                    <button
                      type="submit"
                      className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-glow"
                    >
                      Apply
                    </button>
                  </form>

                  {couponMsg && <div className="text-xs text-indigo-400 mt-2 font-medium">{couponMsg}</div>}

                  {coupon && (
                    <div className="mt-3 p-2.5 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-emerald-300 text-xs flex items-center justify-between">
                      <span className="flex items-center gap-1 font-bold">
                        <Check className="w-3.5 h-3.5" /> Coupon {coupon.code} Applied
                      </span>
                      <button onClick={removeCoupon} className="text-slate-400 hover:text-rose-400 text-[10px] underline">
                        Remove
                      </button>
                    </div>
                  )}
                ...
                <button
                  ...
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 text-white font-bold text-xs uppercase tracking-wider shadow-glow hover:opacity-95 transition-all flex items-center justify-center gap-2"
                >
                  Proceed To Checkout
